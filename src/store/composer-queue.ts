/**
 * Per-session composer prompt queue (ported from Hermes composer-queue).
 *
 * A session holds an ordered list of pending prompts that drain onto the wire
 * one at a time. The pure list ops (enqueue/dequeue/promote/remove/update/
 * migrate/clear) are carried over verbatim; the only substitution is the
 * backing store — Hermes' hand-rolled localStorage nanostore is replaced by our
 * `persistentJsonAtom`, which mirrors to localStorage as JSON best-effort.
 */

import { persistentJsonAtom } from "@/store/atom";
import type { ComposerAttachment } from "@/store/composer-draft";
...
const STORAGE_KEY = "kyrei.composer.queue.v1";

export const $queuedPromptsBySession = persistentJsonAtom<QueueState>(STORAGE_KEY, {});
 ...
 * serializes sends so being edge-free can't double-submit.
 */
export const shouldAutoDrain = ({ isBusy, queueLength }: AutoDrainInput): boolean =>
  !isBusy && queueLength > 0;
