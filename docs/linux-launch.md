...
```bash
# Install runtime deps + optional keyring for API keys (recommended on Wayland WMs)
sudo pacman -S --needed libsecret gnome-keyring xorg-xwayland
...
| Symptom | Fix |
| --- | --- |
| `error: target not found: libappindicator` | You have an **old** package. Rebuild/download **v0.4.5+** (that dep was removed; Arch dropped the package). |
...
```bash
# Зависимости + keyring для сохранения API-ключей (особенно на pure Wayland WM)
sudo pacman -S --needed libsecret gnome-keyring xorg-xwayland
sudo pacman -U ./Kyrei-<version>-x64.pkg.tar.zst
```
...
| Симптом | Что делать |
| --- | --- |
| `error: target not found: libappindicator` | Старый пакет. Нужен **v0.4.5+** (зависимость убрана — в Arch пакета больше нет). |
