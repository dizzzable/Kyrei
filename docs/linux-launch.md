...
```bash
# The native package pulls in libsecret + gnome-keyring automatically.
...
user **inside the graphical session** (not from a bare TTY):
...
| Starts only with X11 session | Same Ozone override; report compositor + `echo $XDG_SESSION_TYPE $WAYLAND_DISPLAY`. |
...
session** that runs Kyrei. The native DEB and Arch packages hard-depend on
`libsecret` and `gnome-keyring`, so the default provider is installed with the
application. A keyring still has to start and unlock inside your login session
before provider keys can be saved.
...
The Arch package hard-depends on `gnome-keyring`. `kwallet` and `keepassxc`
remain optional alternatives for KDE or users who prefer another Secret Service
provider.
...
The DEB hard-depends on `libsecret-1-0` and `gnome-keyring` (plus recommends
`libsecret-tools` for diagnostics). KDE users can install `kwalletmanager` as
an alternative Secret Service provider.
...
An AppImage cannot install a system Secret Service daemon or PAM integration.
It still talks to your session keyring via `libsecret`, so use the native DEB or
Arch package for the out-of-box dependency installation. On AppImage, install
and unlock `gnome-keyring` (or KWallet/KeePassXC) on the host first.

### Where the data lives

Kyrei stores only an encrypted envelope at
`$XDG_CONFIG_HOME/Kyrei/kyrei/kyrei-secrets.json` (normally
`~/.config/Kyrei/kyrei/kyrei-secrets.json`). The encryption key stays in the
Secret Service; inspect Seahorse or KWallet and search for `Kyrei` or
`Chromium Safe Storage`. The JSON file alone cannot decrypt provider keys.
...
```bash
# Нативный пакет сам подтягивает libsecret + gnome-keyring.
sudo pacman -S --needed xorg-xwayland
sudo pacman -U ./Kyrei-<version>-x64.pkg.tar.zst
```

После этого откройте Kyrei из меню приложений или запустите **из графической
сессии** (не из голого TTY):

```bash
kyrei
```

### Arch: установка «не работает»?
...
разблокированный **Secret Service** (`org.freedesktop.secrets`). Нативные DEB-
и Arch-пакеты жёстко зависят от `libsecret` и `gnome-keyring`, поэтому основной
провайдер ставится вместе с приложением. Keyring всё равно должен запуститься
и разблокироваться внутри login-сессии перед сохранением API-ключа.
...
Arch-пакет жёстко зависит от `gnome-keyring`. `kwallet` и `keepassxc` остаются
необязательными альтернативами для KDE и пользователей, которым нужен другой
провайдер Secret Service.
...
DEB жёстко зависит от `libsecret-1-0` и `gnome-keyring` (и рекомендует
`libsecret-tools` для диагностики). Пользователи KDE могут поставить
`kwalletmanager` как альтернативный провайдер Secret Service.
...
AppImage не может установить системный Secret Service или PAM-интеграцию.
Он обращается к keyring сессии через `libsecret`, поэтому для установки
зависимостей «из коробки» используйте DEB или Arch-пакет. Для AppImage сначала
установите и разблокируйте `gnome-keyring` (либо KWallet/KeePassXC) в системе.

### Где лежат данные

Kyrei хранит только зашифрованный контейнер по адресу
`$XDG_CONFIG_HOME/Kyrei/kyrei/kyrei-secrets.json` (обычно
`~/.config/Kyrei/kyrei/kyrei-secrets.json`). Ключ шифрования остаётся в Secret
Service; откройте Seahorse или KWallet и ищите `Kyrei` либо `Chromium Safe
Storage`. Один JSON-файл не позволяет расшифровать ключи провайдеров.
